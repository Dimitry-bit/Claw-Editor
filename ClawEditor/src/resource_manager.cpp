...
void AssetPopType()
{
    assert(assetContext.pushType != ASSET_NONE);
...
bool ResLoadFromFile(const char* identifier, int tags)
{
    switch (assetContext.pushType) {
        ...
        case ASSET_SOUNDBUFFER: {
            ...
            return LoadFromFile<sf::SoundBuffer>(slot,
                                                 soundDir.c_str(),
                                                 identifier);
...
std::vector<asset_slot_t*> ResGetAllAssetSlots(asset_types_t type, const std::regex& regex, int tags)
{
    ...
    switch (type) {
        case ASSET_NONE: {
            ...
            break;
        case ASSET_FONT: {
            table = &assetContext.fontTBL;
        }
            break;
        case ASSET_IMAGE: {
            table = &assetContext.imageTBL;
        }
            break;
        case ASSET_TEXTURE: {
            table = &assetContext.textureTBL;
        }
            break;
        case ASSET_SOUNDBUFFER: {
            table = &assetContext.soundTBL;
        }
            break;
        case ASSET_SPRITESHEET: {
            table = &assetContext.spriteSheetTBL;
        }
            break;
    }
